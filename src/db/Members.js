 const Members = [
    {
        "id": 1,
        name: "Camilo Grisales",
        nickname: "El loko",
        role: "Guitarra - Voz Principal",
        description: `Soy una persona apasionada por la música, el arte y todo lo que transmite emociones reales. Vivo la musica no solo como un arte, sino como una forma de expresar lo que pienso y siento. Hago parte de Pato el Pez, una banda de punk rock  con la que busco conectar con la gente a través de canciones honestas, energía en tarima y momentos que se queden en la memoria.
        También me gusta crear, organizar y mover ideas; desde eventos y shows hasta contenido para redes, siempre buscando que todo tenga identidad propia. Además, disfruto mucho jugar al volley, porque encuentro en el deporte la misma energía, disciplina y adrenalina que vivo en la música. Soy alguien creativo, directo y con ganas de crecer, aprender y hacer que los proyectos en los que creo lleguen cada vez más lejos. Entre ensayos, conciertos, diseño, parches, partidos y sueños, sigo construyendo mi camino a punta de música y pasión.`,
        image: `${import.meta.env.BASE_URL}images/Members/Camilo.webp`,
        gif: `${import.meta.env.BASE_URL}images/GifMembers/LokoGif.gif`
    },
    {
        "id": 2,
        name: "Jaiver Restrepo",
        nickname: "Jai",
        role: "Bajo - Coros",
        description: `Soy una persona apasionada por la música, el arte y todo lo que transmite emociones reales. Vivo la musica no solo como un arte, sino como una forma de expresar lo que pienso y siento. Hago parte de Pato el Pez, una banda de punk rock  con la que busco conectar con la gente a través de canciones honestas, energía en tarima y momentos que se queden en la memoria
        También me gusta crear, organizar y mover ideas; desde eventos y shows hasta contenido para redes, siempre buscando que todo tenga identidad propia. Además, disfruto mucho jugar al volley, porque encuentro en el deporte la misma energía, disciplina y adrenalina que vivo en la música. Soy alguien creativo, directo y con ganas de crecer, aprender y hacer que los proyectos en los que creo lleguen cada vez más lejos. Entre ensayos, conciertos, diseño, parches, partidos y sueños, sigo construyendo mi camino a punta de música y pasión.`,
        image: `${import.meta.env.BASE_URL}images/Members/Jaiver.webp`,
        gif: `${import.meta.env.BASE_URL}images/GifMembers/JaiGif.gif`
    },
    {
        "id": 3,
        name: "Kenny Garzon",
        nickname: "Khaos",
        role: "Guitarra - Coros",
        description: `Diseñador gráfico, editor y músico… básicamente una persona que convirtió trasnochar en parte del proceso creativo. 
        Hago edición de video, animaciones y reparación de computadores. Básicamente soy el tipo que escucha “parce, eso ayer funcionaba perfecto” y automáticamente sabe que le esperan 3 horas de sufrimiento.
        Soy músico y toco guitarra, bajo, requinto, piano y batería.  Fan de Iron Man, los videojuegos y especialmente Call of Duty, donde puedo fallar 38 tiros seguidos… pero igual culpo al lag. 
        Y sí… me duermo a las 4 AM, porque aparentemente mi creatividad trabaja turno nocturno.`,
        image: `${import.meta.env.BASE_URL}images/Members/Kenny.webp`,
        gif: `${import.meta.env.BASE_URL}images/GifMembers/KhaosGif.gif`
    },
    {
        "id": 4,
        name: "Alejandro Quiceno",
        nickname: "Acrux",
        role: "Bateria - Coros",
        description: `Soy Alejandro, aunque muchos me conocen como Acrux. Soy baterista de Pato el Pez y de otros proyectos musicales de la ciudad, donde he encontrado la forma perfecta de transmitir toda la energía, disciplina y pasión que llevo dentro desde que era un niño. La música llegó a mi vida muy temprano y, desde entonces, se convirtió en mucho más que un hobby: es mi manera de sentir, crear y conectar con las personas.
        Sobre el escenario soy de los que creen que cada detalle cuenta. Me apasiona construir presentaciones sólidas, intensas y auténticas, cuidando cada aspecto musical y escénico para que todo ocurra con precisión y personalidad. Siempre estoy buscando evolucionar, aprender algo nuevo y llevar cada proyecto a un nivel más profesional.
        Pero mi creatividad no termina en la música. También soy amante de la programación de software y el diseño web, un mundo donde puedo transformar ideas en experiencias reales. Soy el creador de la página web de Pato el Pez y de otros proyectos digitales que mezclan arte, tecnología e identidad visual. Disfruto aprender constantemente, explorar nuevas herramientas y demostrar que la música y la tecnología pueden hablar el mismo idioma.`,
        image: `${import.meta.env.BASE_URL}images/Members/Alejo.webp`,
        gif: `${import.meta.env.BASE_URL}images/GifMembers/AcruxGif.gif`
    },
    {
        "id": 5,
        name: "Lilith Henao",
        nickname: "Lil",
        role: "Comunity Manager",
        description: `Soy la Community Manager y Manager de la banda. Mientras los demás hacen ruido sobre el escenario, yo trabajo detrás de él, encargándome de la organización, la comunicación y de encontrar nuevas oportunidades para que este proyecto siga creciendo.

        Soy tecnóloga en Gestión del Diseño Textil y de Modas, tengo una media técnica en Gestión y Promoción Artística y actualmente estudio Administración de Empresas, conocimientos que aplico día a día para aportar al desarrollo de la banda. Además de mi trabajo y mis estudios, dedico gran parte de mi tiempo a crear contenido, coordinar ideas y convertir metas en acciones concretas.

        Para mí, la banda es mucho más que un proyecto musical: es un espacio donde la pasión, la creatividad y el compromiso se unen para construir algo auténtico que conecte con las personas dentro y fuera del escenario.`,
        image: `${import.meta.env.BASE_URL}images/Members/Lilith.webp`,
        gif: `${import.meta.env.BASE_URL}images/GifMembers/LilithGif.gif`
    },
    {
        "id": 6,
        name: "Brayan Hincapie",
        nickname: "Bray",
        role: "Productor Multimedia",
        description: `Diseño, Multimedia y Rock & Roll
        Mi trabajo en el equipo creativo es simple, pero visceral: traducir el sonido en un impacto visual salvaje.
        Como diseñador y productor multimedia apasionado por la música, entiendo que el punk rock no solo se escucha, se experimenta en el ser. Mi misión es romper la barrera entre el oído y la vista, plasmando la energía cruda, las distorsiones y la actitud de las canciones en imágenes, videos y estilos que hablen por sí solos.
        No me interesa el diseño limpio o complaciente; busco crear identidades visuales con carácter, texturas y fuerza. Si un arte, un video o un concepto web no te hace sentir la vibración del bajo y la urgencia del mensaje antes de que le des al play, entonces no sirve. Se trata de que la música se pueda ver y que la gente perciba, de un solo golpe de vista, exactamente lo que queremos expresar`,
        image: `${import.meta.env.BASE_URL}images/Members/Bray.webp`,
        gif: `${import.meta.env.BASE_URL}images/GifMembers/BrayanGif.gif`
    }
]
export default Members